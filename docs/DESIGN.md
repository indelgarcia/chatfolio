...
**Post-generation guidance banner (v0.5).** After the portfolio is generated, the left chat column shows a prominent info banner with example prompts grouped into three categories: adjust your portfolio, ask questions, and get recommendations. This is onboarding for the second phase of the experience. Users often assume the tool is "done" once a portfolio appears, so the banner teaches them that iteration is still available.

### What's planned

- Richer profile fields (TODO #4) so the initial onboarding can set clearer expectations about what the assistant will consider
...
**Post-generation chat (v0.3, refined in v0.5).** The chat stays active after portfolio generation. In the current split view, the conversation remains visible in a dedicated left column while the portfolio sits in a dedicated right column. Each column scrolls independently. This matters for user input because iteration is easier when the user can keep the conversation context on screen while reviewing the output they want to change.
...
**Split output layout (v0.5).** After generation, the UI moves into a two-column view where the conversation and portfolio have separate scroll containers. This is a system output improvement as much as a layout change. It lets users inspect the recommendation, rationale, projections, and action plan without losing the assistant responses that explain what changed.
...
**Post-generation edit loop (v0.3, refined in v0.5).** After the portfolio is generated, the user can keep chatting to request changes. The system prompt has specific instructions for this phase - it interprets messages like "make it more aggressive" or "change my budget to $300" as profile adjustment requests and regenerates the portfolio. A spinner and success toast make that regeneration legible, so the user gets immediate feedback that the system heard the request and is updating the output.
